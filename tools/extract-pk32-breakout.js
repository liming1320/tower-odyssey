...
const levels = candidates.filter(x => wanted[x.text.length] > 0 && (wanted[x.text.length]--, true)).map((x, i) => ({ number: i + 1, offset: x.offset, cells: x.text, layoutEncoding: x.text.length === 150 ? '15x10-cells' : 'structured-payload' }));
if (levels.length !== catalog.payloadCount) throw new Error('payload count mismatch: ' + levels.length);
fs.writeFileSync(path.join(__dirname, '..', 'public', 'data', 'pk32-breakout-levels.json'), JSON.stringify({ name: catalog.name, nativeLevelCount: null, extractedPayloadCount: levels.length, controls: catalog.help.filter(x => x.indexOf('鼠标') >= 0), levels }, null, 2) + '\n', 'utf8');
