...
assert.equal(data.name, '黑白棋'); assert.equal(data.count, 18); assert.equal(data.levels.length, 18); assert.equal(data.fullGameRulesVerified, false);
...
assert.ok(data.levels.every(level => level.encoding === 'native-board-string' && /^\d+$/.test(level.cells)));
