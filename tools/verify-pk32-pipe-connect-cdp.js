...
(async () => {
    ...
    try {
        ...
        const result = await cdp.eval(`(async () => {
            ...
            const ready = await waitFor(() => !!host.querySelector('select[aria-label="接水管原版关卡"]') && host.querySelectorAll('.pipe-connect-native-board > button').length === 256); if (!ready) throw new Error('pipe connect launch did not become ready');
            const select = host.querySelector('select[aria-label="接水管原版关卡"]'); const loaded = [];
            for (let index = 0; index < select.options.length; index += 1) { select.value = String(index); select.dispatchEvent(new Event('change', { bubbles: true })); await waitFor(() => host.querySelectorAll('.pipe-connect-native-board > button').length === 256); const prompt = (host.querySelector('.pk32v-prompt') || {}).textContent || ''; loaded.push({ level: index + 1, prompt, cells: host.querySelectorAll('.pipe-connect-native-board > button').length }); }
            const candidate = [...host.querySelectorAll('.pipe-connect-native-board > button')].find(node => node.textContent); const before = candidate ? candidate.textContent : ''; if (candidate) candidate.click(); await new Promise(resolve => setTimeout(resolve, 20)); const after = candidate ? candidate.textContent : '';
            ...
            return { catalogEntry: !!(entry && launch), ready, levelCount: select.options.length, loaded, rotated: !!candidate && before !== after, onlyFiveExposed: select.options.length === 5, overflow, errors, destroyed: !host.querySelector('.pk32v-game') };
        })()`);
        assert.equal(result.catalogEntry, true); assert.equal(result.ready, true); assert.equal(result.levelCount, 5); assert.ok(result.loaded.every(item => item.cells === 256 && item.prompt.includes('1-5'))); assert.equal(result.onlyFiveExposed, true); assert.equal(result.rotated, true); assert.equal(result.overflow, false); assert.deepEqual(result.errors, []); assert.equal(result.destroyed, true);
