...
class TObject(object):
    ...
    def __init__(self, image, offset, image_base):
        ...
        self.method_count = self.tObjectInfo.wMethodCount
        self.method_table_va = self.tObjectInfo.lpMethods
        self.method_table_rva = self.tObjectInfo.lpMethods - image_base
        if self.aMethodNameTable:
            self.name_table_va = self.aMethodNameTable
            self.name_table_rva = self.aMethodNameTable - image_base
        else:
            self.name_table_va = 0
            self.name_table_rva = -1
        # The method pointer table lives in the high-address runtime heap in a
        # live VB5 process. module.bin is a *flat* 33MB dump (offset == RVA) of
        # the structure region; the heap region is usually not captured, so the
        # table VA resolves to an RVA beyond the file -> p-code unavailable.
        self.methods_in_dump = (0 <= self.method_table_rva < len(image))
        self.methods = []
        if self.methods_in_dump:
            ...
            for i in range(self.tObjectInfo.wMethodCount):
                ...
                proc_rva = proc_va - image_base
                if 0 <= proc_rva < len(image):
                    proc = ProcDscInfo(image, proc_rva, image_base)
                    proc.name = mname
...
class TObjectInfo(object):
    # 56-byte ObjectInfo: HH + 7*L + HH + L + HH + 3*L
    FMT = "<HHLLLLLLLHHLLLLL"
...
def run(bin_path, out_dir):
    prog = os.path.join(out_dir, "_RUN.log")
    def log(*a):
        with open(prog, "a", encoding="utf-8") as lf:
            lf.write(" ".join(str(x) for x in a) + "\n")
    log("START bin=%s out=%s" % (bin_path, out_dir))
    pe = PEReader(bin_path)
    image = pe.build_flat_image()
    log("ImageBase=0x%X file_size=%d" % (pe.ImageBase, len(image)))
    vb_off = find_vb_header(image)
    if vb_off < 0:
        log("VB5! signature not found")
        return None
    log("VB5! at RVA 0x%X (file off 0x%X)" % (vb_off, vb_off))
    hdr = VbHeader(image, vb_off, pe.ImageBase)
    log("Signature=%r RuntimeBuild=%d LanguageDLL=%s FormCount=%d"
        % (hdr.Signature, hdr.RuntimeBuild, hdr.LanguageDLL, hdr.FormCount))
    log("ProjectExename=%r ProjectTitle=%r ProjectName=%r"
        % (hdr.oProjectExename, hdr.oProjectTitle, hdr.oProjectName))

    proj = ProjectInfo(image, hdr.aProjectInfo - pe.ImageBase, pe.ImageBase)
    log("aObjectTable=0x%X iObjectsCount=%d aNativeCode=0x%X lExternalCount=%d"
        % (proj.aObjectTable, proj.objectTable.iObjectsCount,
           proj.aNativeCode, proj.lExternalCount))
    ...
    for oi, obj in enumerate(proj.objectTable.objects):
        ...
        with open(fpath, "w", encoding="utf-8") as fo:
            fo.write("OBJECT #%d\n" % oi)
            fo.write("  Name           : %s\n" % oname)
            fo.write("  DeclaredMethods: %d (TObject.lMethodCount=%d, TObjectInfo.wMethodCount=%d)\n"
                     % (obj.method_count, obj.lMethodCount, obj.tObjectInfo.wMethodCount))
            fo.write("  ObjectType     : 0x%X\n" % obj.lObjectType)
            fo.write("  aObjectInfo    : 0x%X\n" % obj.aObjectInfo)
            fo.write("  MethodTableVA  : 0x%X  (RVA 0x%X)\n"
                     % (obj.method_table_va, obj.method_table_rva))
            nrva = ("0x%X" % obj.name_table_rva) if obj.name_table_rva >= 0 else "n/a"
            fo.write("  NameTableVA    : 0x%X  (RVA %s)\n"
                     % (obj.name_table_va, nrva))
            fo.write("  FileSize       : 0x%X (%d bytes)\n" % (len(image), len(image)))
            if not obj.methods_in_dump:
                fo.write("\n  *** METHOD BODIES NOT IN DUMP ***\n")
                fo.write("  The method pointer table at RVA 0x%X lies beyond the\n"
                         "  %d-byte flat dump. This module.bin captured the VB5\n"
                         "  structure/metadata but not the high-address runtime\n"
                         "  heap where p-code actually lives. No disassembly\n"
                         "  possible from this file alone.\n"
                         % (obj.method_table_rva, len(image)))
                fo.write("\n")
            else:
                fo.write("\n")
                mcount = 0
                for mi, m in enumerate(obj.methods):
                    ...
                    if m.ProcSize > 0:
                        ...
                        for (off, name, consumed, raw) in dis:
                            ...
                            fo.write("    +%04X  %-22s %s\n" % (off, name, hexs))
                        fo.write("\n")
        index_lines.append("%03d  %-34s methods=%-4d in_dump=%-3s type=0x%X"
                           % (oi, oname, obj.method_count,
                              "Y" if obj.methods_in_dump else "N", obj.lObjectType))
    ...
    in_dump = sum(1 for o in proj.objectTable.objects if o.methods_in_dump)
    log("DONE wrote %d object dumps; %d/%d have method tables in dump"
        % (len(proj.objectTable.objects), in_dump,
           len(proj.objectTable.objects)))
    return proj
...
if __name__ == "__main__":
    ...
    try:
        ...
        errlog = os.path.join(HERE, "vb5-pcode-dump.err.log")
