...
const result = {
    ...
    summary: {
        ...
        helpTextEvidence: records.filter(record => record.migrationEvidence && record.migrationEvidence.helpTextEvidenceBound).length,
        sharedPcodeMetadata: records.filter(record => record.migrationEvidence && record.migrationEvidence.sharedPcodeMetadataBound).length,
        sharedRuntimeMethodTable: records.filter(record => record.migrationEvidence && record.migrationEvidence.sharedRuntimeMethodTableBound).length,
        runtimeMethodEntries: records.reduce((max, record) => Math.max(max, record.migrationEvidence && record.migrationEvidence.runtimeMethodEntries || 0), 0),
        runtimeMethodRegionTargets: records.reduce((max, record) => Math.max(max, record.migrationEvidence && record.migrationEvidence.runtimeMethodRegionTargets || 0), 0),
        sharedRuntimePcodeSlices: records.filter(record => record.migrationEvidence && record.migrationEvidence.sharedRuntimePcodeSlicesBound).length,
        runtimePcodeSlices: records.reduce((max, record) => Math.max(max, record.migrationEvidence && record.migrationEvidence.runtimePcodeSlices || 0), 0),
        runtimePcodeTrustedSlices: records.reduce((max, record) => Math.max(max, record.migrationEvidence && record.migrationEvidence.runtimePcodeTrustedSlices || 0), 0),
        runtimePcodeTerminatedSlices: records.reduce((max, record) => Math.max(max, record.migrationEvidence && record.migrationEvidence.runtimePcodeTerminatedSlices || 0), 0),
        runtimePcodeTrustedTerminatedSlices: records.reduce((max, record) => Math.max(max, record.migrationEvidence && record.migrationEvidence.runtimePcodeTrustedTerminatedSlices || 0), 0),
        methodBodyCaptured: records.some(record => record.migrationEvidence && record.migrationEvidence.methodBodyCaptured) ? 1 : 0,
        charGridCandidateGames: records.filter(record => record.migrationEvidence && record.migrationEvidence.charGridCandidatesBound).length,
        charGridCandidates: records.reduce((sum, record) => sum + (record.migrationEvidence && record.migrationEvidence.charGridCandidateCount || 0), 0),
        candidateDataRenderer: records.filter(record => record.migrationEvidence && record.migrationEvidence.candidateDataRendererBound).length,
        ...
        evidenceAdapterMigration: records.filter(record => record.migrationPhase === 'evidence-adapter-migration').length,
        catalogMigration: records.filter(record => record.migrationPhase === 'catalog-migration').length,
