...
const GAMES = ['tower24', 'cookingfever'];
...
(async () => {
    ...
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cdpmt-'));
    ...
    await cdp.eval(`(async () => {
        const n = 'mtchk' + Date.now().toString().slice(-5);
    ...
    for (const id of GAMES) {
        ...
        // 魔塔：往右走两步制造画面内容
        if (id === 'tower24') {
            await cdp.eval(`(() => { const s = window.__MG_TEST || (MG && MG._inst); return true; })()`);
            await cdp.eval(`dispatchEvent(new KeyboardEvent('keydown', { key: 'd' }))`);
            await sleep(400);
            await cdp.eval(`dispatchEvent(new KeyboardEvent('keyup', { key: 'd' }))`);
            await sleep(600);
        }
