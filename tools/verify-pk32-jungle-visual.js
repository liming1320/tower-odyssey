  ...
  try {
    for (const width of [390, 1280]) {
      ...
      const result = await page.evaluate(() => {
        ...
        return { cells: grid ? grid.children.length : 0, pieces: pieces.length, ranks: pieces.map(node => node.dataset.rank), labels: pieces.map(node => node.textContent), terrain: [...document.querySelectorAll('.pk32-animal-cell')].map(node => node.dataset.terrain), nativeMask: pieces.map(node => getComputedStyle(node).webkitMaskImage), scrollable: !!shell && shell.scrollWidth >= shell.clientWidth };
      });
      assert.equal(result.cells, 63);
      assert.equal(result.pieces, 16);
      assert.equal(result.terrain.filter(value => value === 'river').length, 12);
      assert.equal(result.terrain.filter(value => value === 'trap').length, 6);
      assert.equal(result.terrain.filter(value => value === 'den').length, 2);
      assert.ok(result.ranks.every(rank => /^[1-8]$/.test(rank)));
        assert.deepEqual(new Set(result.labels), new Set(['象', '狮', '虎', '豹', '狼', '狗', '猫', '鼠']));
      assert.ok(result.nativeMask.every(image => image === 'none' || image === ''));
