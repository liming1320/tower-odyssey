...
(async () => {
    ...
    try {
        ...
        check('魔塔显示 11×11 地图', tower.cells === 121, tower.cells); check('魔塔有语义对象提示', tower.titles.some(x => /墙|主角|怪物|钥匙|门|出口/.test(x)), tower.titles.join(' | ')); check('魔塔资源来源已声明', tower.assetSource === 'pk32-tower-sheet', tower.assetSource); check('魔塔资源映射状态诚实', tower.assetStatus === 'mapping-incomplete', tower.assetStatus); check('魔塔不渲染坦克或其他图集', tower.wrongImages === 0, tower.wrongImages); check('魔塔对象类型可区分', tower.kinds.length >= 6, tower.kinds.join(','));
        ...
        check('强手棋显示 40 格路线', richman.cells === 40, richman.cells); check('强手棋格子坐标不重叠', richman.uniqueCoords === 40, richman.uniqueCoords); check('强手棋显示 3 名玩家', richman.players === 3, richman.players); check('强手棋操作按钮齐全', richman.buttons >= 6, richman.buttons); check('强手棋提供放弃购买', richman.hasPass, richman.hasPass); check('强手棋不加载综合图集', !/board-sheet/.test(richman.image), richman.image); check('强手棋接入原始棋盘编码', richman.sourceLayout.length === 392, richman.sourceLayout.length); check('强手棋接入原始事件表', richman.sourceEvents >= 30, richman.sourceEvents); check('强手棋掷骰子后玩家移动', richman.afterRoll.players[0].pos === 2 && richman.afterRoll.phase === 'buy', JSON.stringify(richman.afterRoll)); check('强手棋购买地产后扣款并归属', richman.afterBuy.own[2] === 0 && richman.afterBuy.players[0].cash === 2740, JSON.stringify(richman.afterBuy)); check('强手棋电脑回合可收束', richman.afterComputer.round === 2 && richman.afterComputer.phase === 'roll' && richman.afterComputer.turn === 0, JSON.stringify(richman.afterComputer)); check('强手棋交互无运行时错误', richman.errors.length === 0, richman.errors.join(' | '));
