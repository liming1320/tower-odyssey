...
const levels = candidates.map((x, i) => ({ number: i + 1, offset: x.offset, cells: x.text }));
if (levels.length !== 70) throw new Error('payload count mismatch: ' + levels.length);
fs.writeFileSync(path.join(__dirname, '..', 'public', 'data', 'pk32-castle-levels.json'), JSON.stringify({ name: catalog.name, nativeLevelCount: 100, extractedPayloadCount: levels.length, rules: catalog.help.slice(1), levels }, null, 2) + '\n', 'utf8');
