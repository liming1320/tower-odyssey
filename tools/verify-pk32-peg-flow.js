...
(async () => {
    ...
    const result = await page.evaluate(async () => {
        ...
        const occupied = i => cells[i].dataset.code !== '0' && cells[i].dataset.code !== '6';
        let move = null;
        for (let i = 0; i < 280 && !move; i++) {
            if (!occupied(i)) continue;
            const row = Math.floor(i / 20), col = i % 20;
            for (const [dr, dc] of [[-1, 0], [1, 0], [0, -1], [0, 1]]) {
                const middle = (row + dr) * 20 + col + dc;
                const target = (row + dr * 2) * 20 + col + dc * 2;
                if (row + dr * 2 >= 0 && row + dr * 2 < 14 && col + dc * 2 >= 0 && col + dc * 2 < 20 && occupied(middle) && cells[target].dataset.code === '6') {
    ...
    assert.equal(result.cells, 280);
    assert.equal(result.source, '6');
    assert.equal(result.middle, '6');
    assert.equal(result.target, result.move.sourceCode);
    assert.equal(result.level, true);
    assert.deepEqual(errors, []);
    await browser.close();
    console.log(JSON.stringify({ game: '独粒钻石', nativeLevels: 207, legalMove: result.move, preservesPieceCode: true, errors }));
