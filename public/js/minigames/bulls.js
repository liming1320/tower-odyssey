        ...
        const render = () => {
            wrap.innerHTML = `
                ...
                <div style="display:flex;gap:6px;margin-bottom:12px">
                    <input id="g-in" maxlength="${digits}" placeholder="${digits} 位" style="flex:1;padding:8px;border-radius:6px;border:1px solid #555;background:#2a2540;color:#fff;font-size:18px;letter-spacing:6px;text-align:center">
                    <button id="g-go" class="mg-btn" style="margin:0;padding:8px 16px">猜</button>
                </div>
                <div id="g-list" style="font-size:13px;line-height:1.8;max-height:60vh;overflow-y:auto">${history.map((h, i) => `<div>${i + 1}. <b style="color:#ffd56b">${h.g}</b> → <span style="color:#5cd65c">${h.a}A${h.b}B</span></div>`).join('') || '<div style="color:#888">还没有猜测</div>'}</div>
                ${over ? '<div style="margin-top:12px;color:#ffd56b;font-size:14px">答案：<b>' + secret.join('') + '</b></div>' : ''}`;
