...
(function () {
    ...
    function startGame(container, name, opts) {
        ...
        function renderGo() {
            const size = 9; const cells = Array(size * size).fill(0); let turn = 1, passes = 0; const grid = renderGrid(size, size, 'go-board');
            ...
            function draw() { grid.innerHTML = ''; cells.forEach(function (v, i) { const b = button(v === 1 ? '●' : v === 2 ? '○' : '·', function () { if (v || ended) return; const copy = cells.slice(); copy[i] = turn; neighbors(i).filter(function (n) { return copy[n] === 3 - turn; }).forEach(function (n) { const g = group(n, copy); if (!g.liberties.size) g.stones.forEach(function (x) { copy[x] = 0; }); }); const own = group(i, copy); if (!own.liberties.size) return status.textContent = '禁着：不能自杀'; cells.splice(0, cells.length); copy.forEach(function (x) { cells.push(x); }); passes = 0; turn = 3 - turn; setScore(score + 1); status.textContent = '围棋九路棋盘，轮到' + (turn === 1 ? '黑' : '白') + '方'; draw(); }); grid.appendChild(b); }); }
            const pass = button('停着', function () { passes++; turn = 3 - turn; if (passes >= 2) return finish('双方连续停着，本局结束。'); status.textContent = '停着后轮到' + (turn === 1 ? '黑' : '白') + '方'; });
        ...
        function renderMilitary() {
            ...
            function draw() { grid.innerHTML = ''; pieces.forEach(function (p, i) { const b = button(text(p), function () { if (!p || p.side === (turn ? '蓝' : '红')) return; if (!p.face) { p.face = true; status.textContent = '翻开了' + p.side + '方棋子；轮到' + (turn ? '红' : '蓝') + '方'; draw(); return; } if (selected < 0) { if (p.side === (turn ? '红' : '蓝')) selected = i; } else { const own = pieces[selected]; const dx = Math.abs(i % size - selected % size), dy = Math.abs(Math.floor(i / size) - Math.floor(selected / size)); if (p.side === own.side || dx + dy !== 1) { selected = p.side === own.side ? i : -1; } else if (!p.face) { p.face = true; selected = -1; } else { const wins = own.rank === 0 ? false : p.rank === 0 || own.rank >= p.rank; if (wins) { pieces[i] = own; pieces[selected] = null; if (p.rank === 0) return finish(own.side + '方夺取军旗，获得胜利。'); } else if (own.rank === p.rank) pieces[selected] = null; else pieces[selected] = p; selected = -1; turn = 1 - turn; setScore(score + 1); } } draw(); }); if (selected === i) b.classList.add('selected'); grid.appendChild(b); }); }
        ...
        function renderBilliards() {
            const grid = renderGrid(6, 4, 'billiards-table'); let balls = 9; const cue = button('击球', function () { if (!balls) return; balls -= 1; setScore(score + 10); status.textContent = config.title + '：剩余目标球 ' + balls; if (!balls) finish('清台完成，本局结束。'); });
            body.append(el('p', { className: 'pk32v-prompt' }, '独立台球流程：选择击球并逐个清台；不同名称保留不同目标球数。'), grid, cue);
            for (let i = 0; i < 24; i += 1) grid.appendChild(button(i < balls ? '●' : '○', function () {}));
        }
        function renderBubble() {
            ...
            function draw() { grid.innerHTML = ''; values.forEach(function (value, i) { const b = button(value || '·', function () { if (!value || ended) return; const hit = group(i); if (hit.length < 2) return; hit.forEach(function (n) { values[n] = ''; }); collapse(); for (let n = 0; n < size * size; n += 1) if (!values[n]) values[n] = colors[randomInt(colors.length)]; setScore(score + hit.length * 3); draw(); if (!hasMoves()) finish('没有可消除的相邻球组，本局结束。'); }); b.disabled = !value; grid.appendChild(b); }); prompt.textContent = '当前得分 ' + score + '；点击相邻同色球组消除'; }
