...
(function () {
    ...
    function startGame(container, name, opts) {
        ...
        function renderNumber() {
            let target = 1 + randomInt(9); const total = config.levelCount || 1; const prompt = el('p', { className: 'pk32v-prompt' }, (config.name === '数谜' ? '原版关卡：第 1 / ' + total + ' 关；当前已确认原生数据串 ' + config.nativePayloadCount + ' 条。' : '') + '找出目标数字：' + target); const grid = el('div', { className: 'pk32v-grid' });
        ...
        function renderNativeNextHundred() {
            ...
            fetch('/data/pk32-next-hundred-levels.json').then(function (response) { return response.json(); }).then(function (data) { nativeData = data; const select = el('select', { 'aria-label': '下一百层原生载荷' }); data.levels.forEach(function (item, index) { select.appendChild(el('option', { value: String(index) }, '原生数据 ' + (index + 1))); }); select.onchange = function () { nativeLevel = Number(select.value) || 0; draw(); }; panel.appendChild(select); draw(); }).catch(function () { prompt.textContent = '原生载荷加载失败，使用规则验证盘面。'; }); startTimer(); draw();
        }
