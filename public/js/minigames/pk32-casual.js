...
(function () {
    ...
    function pipes(host, title) { let grid, over; const size = 4; function hasBoundaryLeak() { return grid.some(function (r, i) { const x = i % size, y = Math.floor(i / size), out = links(i); return (x === 0 && out.indexOf('L') >= 0) || (x === size - 1 && out.indexOf('R') >= 0) || (y === 0 && out.indexOf('U') >= 0) || (y === size - 1 && out.indexOf('D') >= 0); }); } function init() { let attempts = 0; do { grid = Array.from({ length: size * size }, function () { return Math.floor(Math.random() * 4); }); attempts += 1; } while (attempts < 5000 && (hasBoundaryLeak() || connected())); over = false; draw(); } function links(i) { const r = grid[i], out = []; if (r === 0 || r === 1) out.push('D'); if (r === 1 || r === 2) out.push('R'); if (r === 2 || r === 3) out.push('U'); if (r === 3 || r === 0) out.push('L'); return out; } function connected() { const seen = new Set([0]), queue = [0]; while (queue.length) { const i = queue.shift(), x = i % size, y = Math.floor(i / size); links(i).forEach(function (dir) { const nx = x + (dir === 'R' ? 1 : dir === 'L' ? -1 : 0), ny = y + (dir === 'D' ? 1 : dir === 'U' ? -1 : 0); if (nx < 0 || ny < 0 || nx >= size || ny >= size) return; const n = ny * size + nx, opposite = { R: 'L', L: 'R', U: 'D', D: 'U' }[dir]; if (links(n).indexOf(opposite) >= 0 && !seen.has(n)) { seen.add(n); queue.push(n); } }); } return seen.has(size * size - 1); } function draw() { const area = shell(host, title, button('重置', 'reset'), over ? '管道已接通，点击重置再来一局' : '旋转管道，令左上角连接到右下角'); area.innerHTML += '<div style="display:grid;grid-template-columns:repeat(4,48px);gap:3px;margin-top:10px">' + grid.map(function (r, i) { return '<button class="btn ghost" style="width:48px;height:48px;font-size:24px" data-p="' + i + '">' + ['└','┌','┐','┘'][r] + '</button>'; }).join('') + '</div>'; area.querySelector('[data-action="reset"]').onclick = init; area.querySelectorAll('[data-p]').forEach(function (b) { b.disabled = over; b.onclick = function () { if (over) return; grid[+b.dataset.p] = (grid[+b.dataset.p] + 1) % 4; if (connected()) over = true; draw(); }; }); } init(); }
