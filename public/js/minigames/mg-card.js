...
(function () {
    ...
    // ============ 8. 大富翁（简易）============
    const MP = [
        { n: '起点', t: 0 }, { n: '布庄', t: 1, p: 60, r: 12 }, { n: '机会', t: 2 }, { n: '米行', t: 1, p: 80, r: 16 },
        { n: '茶馆', t: 1, p: 100, r: 20 }, { n: '监狱', t: 3 }, { n: '酒楼', t: 1, p: 120, r: 24 }, { n: '骰运', t: 2 },
        { n: '钱庄', t: 1, p: 140, r: 28 }, { n: '镖局', t: 1, p: 160, r: 32 }, { n: '免费停', t: 4 },
        { n: '马市', t: 1, p: 180, r: 36 }, { n: '药铺', t: 1, p: 200, r: 40 }, { n: '当铺', t: 1, p: 220, r: 44 },
    ];
    E.def('monopoly', {
        levels: E.nm(),
        params: (i, t) => ({ start: 1500, goal: 3000 + i * 400 }),
        w: 396, h: 470,
        hint: '掷骰前进，买地收租，先攒到目标资产即胜',
        init: P => ({
            cells: MP, pp: 0, ap: 0, cash: P.start, acash: P.start,
            own: new Array(MP.length).fill(0), goal: P.goal, msg: '掷骰开始', rolled: false,
        }),
        draw(ctx, S, P, W, H) {
            E.bg(ctx, W, H, '#2f4a3a', '#12241c');
            for (let i = 0; i < S.cells.length; i++) {
                const a = -Math.PI / 2 + i * (Math.PI * 2 / S.cells.length);
                const cx = W / 2 + Math.cos(a) * 130, cy = 210 + Math.sin(a) * 110;
                const c = S.cells[i];
                E.card(ctx, cx - 26, cy - 16, 52, 32, c.t === 1 ? '#8a7a4a' : '#4a5a7a', c.t === 1 ? '#5a4a2a' : '#2a3a5a', 5);
                E.txt(ctx, c.n, cx, cy - 4, 11, '#fff', true);
                if (c.t === 1) E.txt(ctx, S.own[i] ? (S.own[i] === 1 ? '💰我' : '💸敌') : '¥' + c.p, cx, cy + 9, 10, S.own[i] ? '#ffd56b' : '#e8e0c0');
            }
            const pa = -Math.PI / 2 + S.pp * (Math.PI * 2 / S.cells.length);
            ctx.beginPath(); ctx.arc(W / 2 + Math.cos(pa) * 130, 210 + Math.sin(pa) * 110, 10, 0, 6.284);
            ctx.fillStyle = '#5cc7ff'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
            const aa = -Math.PI / 2 + S.ap * (Math.PI * 2 / S.cells.length);
            ctx.beginPath(); ctx.arc(W / 2 + Math.cos(aa) * 130, 210 + Math.sin(aa) * 110, 10, 0, 6.284);
            ctx.fillStyle = '#ff7a8b'; ctx.fill(); ctx.strokeStyle = '#fff'; ctx.lineWidth = 2; ctx.stroke();
            E.txt(ctx, `💰 你 ${S.cash}　电脑 ${S.acash}　目标 ${S.goal}`, W / 2, 30, 15, '#ffd56b', true);
            E.btnBox(ctx, 40, H - 90, 140, 48, '掷骰/确认', '#8a5a2f', '#5a3a1c');
            const cur = S.cells[S.pp];
            if (cur.t === 1 && !S.own[S.pp] && S.cash >= cur.p) E.btnBox(ctx, 200, H - 90, 140, 48, '买下 ' + cur.p, '#2f7f4a', '#1a4a2a');
            E.txt(ctx, S.msg, W / 2, H - 24, 14, '#cfe8d8');
        },
        tap(S, x, y, P, api) {
            const H = 470;
            if (E.hit(x, y, 40, H - 90, 140, 48)) { mpRoll(S, api); return; }
            const cur = S.cells[S.pp];
            if (cur.t === 1 && !S.own[S.pp] && S.cash >= cur.p && E.hit(x, y, 200, H - 90, 140, 48)) {
                S.cash -= cur.p; S.own[S.pp] = 1; S.msg = `买下 ${cur.n}！`; mpEnd(S, api);
            }
        },
    });
    function mpRoll(S, api) {
        const d1 = ri(1, 6), d2 = ri(1, 6);
        S.pp = (S.pp + d1 + d2) % S.cells.length;
        const c = S.cells[S.pp];
        if (c.t === 0) { S.cash += 200; S.msg = `掷 ${d1 + d2} 经过起点 +200`; }
        else if (c.t === 2) { const v = ri(-100, 200); S.cash += v; S.msg = `机会：${v >= 0 ? '+' : ''}${v}`; }
        else if (c.t === 1) {
            if (S.own[S.pp] === 2) { S.cash -= c.r; S.acash += c.r; S.msg = `掷 ${d1 + d2} 踩到电脑的地，付租 ${c.r}`; }
            else if (!S.own[S.pp]) S.msg = `掷 ${d1 + d2} 到达 ${c.n}（${c.p}）可购买`;
            else S.msg = `掷 ${d1 + d2} 回到自己的地`;
        } else S.msg = `掷 ${d1 + d2} ${c.n}`;
        // 电脑回合
        const e1 = ri(1, 6), e2 = ri(1, 6);
        S.ap = (S.ap + e1 + e2) % S.cells.length;
        const ec = S.cells[S.ap];
        if (ec.t === 0) S.acash += 200;
        else if (ec.t === 2) S.acash += ri(-100, 200);
        else if (ec.t === 1) {
            if (S.own[S.ap] === 1) { S.acash -= ec.r; S.cash += ec.r; S.msg += `｜电脑付你租 ${ec.r}`; }
            else if (!S.own[S.ap] && S.acash >= ec.p) { S.acash -= ec.p; S.own[S.ap] = 2; S.msg += '｜电脑买地'; }
        }
        mpEnd(S, api);
    }
    function mpEnd(S, api) {
        if (S.cash >= S.goal) return api.finish({ win: true, stars: 3, score: S.cash, lines: [`资产达到 ${S.cash}`] });
        if (S.cash < 0) return api.finish({ win: false, stars: 0, score: 0, lines: ['破产了'] });
        if (S.acash < 0) return api.finish({ win: true, stars: 3, score: S.cash, lines: ['电脑破产，你赢！'] });
    }
})();
