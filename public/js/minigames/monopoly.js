...
(function () {
    ...
    function buildTiles() {
        for (let i = 0; i < N; i++) {
            ...
            const top = new THREE.Mesh(new THREE.PlaneGeometry(0.9, 0.9), new THREE.MeshStandardMaterial({ map: tileTexture(c, i), roughness: 0.96 }));
    ...
    function start3D(container) {
        return ensureThree().then(T => {
            ...
            const sun = new THREE.DirectionalLight(0xfff2d8, 1.05); sun.position.set(8, 17, 6); sun.castShadow = true;
            ...
            buildTiles(); buildBuildings(); buildPawns(); buildCenter(); buildMarks(); buildDice();
