...
(function (global) {
    ...
    function start(container, opts) {
        ...
        var root = el('section', 'pk32-pk-richman'); var head = el('header', 'pk32-rh-head'); var title = el('h2', '', 'PK32 · 强手棋原版迁移'); var status = el('div', 'pk32-rh-status'); var board = el('div', 'pk32-rh-board'); board.dataset.pk32Layout = BOARD_MAP; var actions = el('div', 'pk32-rh-actions'); var log = el('div', 'pk32-rh-log');
        head.append(title, status); root.append(head, board, actions, log); container.appendChild(root);
        var style = document.createElement('style'); style.textContent = '.pk32-pk-richman{max-width:1040px;margin:auto;padding:14px;background:#073b3d;color:#f8f2d8;border:3px solid #c99843;font-family:system-ui}.pk32-rh-head{display:flex;justify-content:space-between;gap:10px;align-items:center}.pk32-rh-status{color:#ffe08a}.pk32-rh-board{position:relative;display:grid;grid-template-columns:repeat(12,minmax(36px,1fr));grid-template-rows:repeat(12,58px);gap:2px;margin:12px auto;padding:10px;background:#0b5554 url("/img/pk32/board-sheet.png") center/cover;max-width:1000px;min-height:720px}.pk32-rh-cell{position:relative;display:flex;flex-direction:column;justify-content:space-between;padding:4px;background:rgba(0,90,88,.88);border:1px solid #d8c782;color:#fff;font-size:11px;text-align:center;cursor:pointer}.pk32-rh-cell .bar{height:6px;margin:-4px -4px 2px;background:#777}.pk32-rh-cell .price{color:#ffe08a}.pk32-rh-pips{display:flex;justify-content:center;gap:2px;min-height:22px;flex-wrap:wrap}.pk32-rh-pip{width:20px;height:20px;border-radius:50%;display:grid;place-items:center;color:#102b2c;border:2px solid #fff;font-size:12px}.pk32-rh-actions{display:flex;gap:8px;flex-wrap:wrap}.pk32-rh-actions button{padding:8px 13px;background:#155e60;color:#fff;border:1px solid #d8c782;border-radius:4px;cursor:pointer}.pk32-rh-log{min-height:54px;margin-top:10px;padding:8px;background:rgba(0,0,0,.2);white-space:pre-wrap}.pk32-rh-panel{display:grid;grid-template-columns:repeat(3,1fr);gap:5px;margin:10px 0}.pk32-rh-player{padding:6px;border:1px solid #628d83;background:rgba(0,0,0,.15)}.pk32-rh-hand{display:flex;gap:6px;flex-wrap:wrap;margin:8px 0}.pk32-rh-hand button{padding:6px 9px;background:#145e60;color:#fff;border:1px solid #d8c782;border-radius:4px;cursor:pointer}'; root.appendChild(style);
