...
(function (global) {
    'use strict';
    var N = 40, INITIAL_CASH = 10000, SAVE = 'pk32-richman-save-picform13-v2';
    ...
    function start(container, opts) {
        ...
        function movePlayer(pi, done) { var p = s.players[pi]; if (s.jail[pi] > 0) { s.jail[pi] -= 1; s.phase = 'end'; s.message = p.name + ' 在监狱中，跳过本回合'; render(); if (done) done(); else if (pi === 0) queueAITurns(); return; } var d = 1 + Math.floor(Math.random() * 6), steps = 0; s.dice = [d]; s.phase = 'moving'; s.message = p.name + ' 掷出 ' + d + ' 点'; render(); function step() { if (stopped) return; if (steps >= d) { land(pi); if (pi === 0) selected = p.pos; gameOver(); render(); if (done) done(); else if (pi === 0 && s.phase === 'end') queueAITurns(); return; } var previous = p.pos; p.pos = (p.pos + 1) % N; steps += 1; if (p.pos < previous) { p.cash += 2000; if (s.spy[pi] > 0) s.spy[pi] -= 1; } s.message = p.name + ' 移动第 ' + steps + ' / ' + d + ' 格'; render(); setTimeout(step, moveDelay); } step(); }
